...
create table ticket (
ticket_id int(10) not null primary Key auto_increment, comment'チケットID',
ticket_name varchar(255) not null comment'チケットの名前',
artist_name varchar(255) not null comment'アーティスト名',
day date not null comment'日',
detail text not null comment'説明書',
stock int(10) not null comment'ストック',
unit_price int(10) not null comment'団体料金'
);


drop table if exists user;
create table user (  /* ユーザーのテーブルを作る */
user_id int not null auto_increment comment'ユーザーID',/* ユーザーに番号を割り振る、自動連番 */
